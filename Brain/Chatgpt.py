...
def MainExecutionChatGPT():
    ...
    with open("Database/chatGpt/current_reply.txt","w") as rq:
        rq.write("")
        rq.close()
    FileWriter(Data='3')
    prevQuery = ""
    while True:
        try:
            with open("Database/current_query.txt","r") as f:
                Query = f.read()
                f.close()
                if prevQuery != Query:
                    print("Query: ", Query)
                    reply = ChatGPTBrain(Query=Query)
                    print(reply)
                    with open("Database/chatGpt/current_reply.txt","w") as file:
                        file.write(reply)
                        file.close()
                    prevQuery = Query
                else:
